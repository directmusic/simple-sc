...
void start_pipewire_capture(int pipewire_fd, uint32_t node_id) {
    ...
    if (pw_stream_connect(data.stream, PW_DIRECTION_INPUT, node_id, (pw_stream_flags)(PW_STREAM_FLAG_AUTOCONNECT | PW_STREAM_FLAG_MAP_BUFFERS), video_params, 1) != 0) {
...
int main(int argc, char* argv[]) {
    ...
    if (pw_stream_connect(data.stream, PW_DIRECTION_INPUT, portal_data.node_id, (pw_stream_flags)(PW_STREAM_FLAG_AUTOCONNECT | PW_STREAM_FLAG_MAP_BUFFERS), video_params, 1) != 0) {
        printf("Couldn't connect to video stream\n");
    }
